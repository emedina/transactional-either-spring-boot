...
public abstract class TransactionAspectSupportWithEither implements BeanFactoryAware, InitializingBean {
    ...
    private static final boolean reactiveStreamsPresent = ClassUtils.isPresent("org.reactivestreams.Publisher",
        TransactionAspectSupportWithEither.class.getClassLoader());
     ...
     * @see TransactionAspectSupportWithEither.TransactionInfo#hasTransaction()
    ...
    protected TransactionAspectSupportWithEither() {
    ...
    protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
        ...
        if (this.reactiveAdapterRegistry != null && tm instanceof ReactiveTransactionManager rtm) {
            ...
            CoroutinesInvocationCallback corInv = (isSuspendingFunction ?
                (CoroutinesInvocationCallback) invocation :
                null);

            ReactiveTransactionSupport txSupport = this.transactionSupportCache.computeIfAbsent(method, key -> {
                Class<?> reactiveType = (isSuspendingFunction ?
                    (hasSuspendingFlowReturnType ? Flux.class : Mono.class) :
                    method.getReturnType());
                // Check whether reactive type is an Either, in which case we need to unwrap it.
            ...
            if (corInv != null) {
                callback = () -> CoroutinesUtils.invokeSuspendingFunction(method, corInv.getTarget(), corInv
                    .getArguments());
            }
            ...
            try {
                result = cpptm.execute(txAttr, status -> {
                    ...
                    try {
                        ...
                        if (retVal != null && vavrPresent && VavrDelegate.isVavrEither(retVal)) {
                            // Set rollback-only in case of Vavr failure matching our rollback rules...
                            retVal = VavrDelegate.evaluateEitherLeft(retVal, txAttr, status);
                        }
                        return retVal;
    ...
    public static class VavrDelegate {
         ...
         */
        public static Either<?, ?> evaluateEitherLeft(Object retVal, TransactionAttribute txAttr,
                    ...
                    if (errors instanceof Iterable) {
                        for (Object error : (Iterable<?>) errors) {
                            if (error instanceof Throwable && txAttr.rollbackOn((Throwable) error)) {
                                ...
                                && ((RuleBasedTransactionAttributeWithEither) txAttr).rollbackOnErrorValue(error
                                    .getClass())) {
                                status.setRollbackOnly();
                                break;
                            }
                    ...
                    if (errors instanceof Iterable) {
                        for (Object error : (Iterable<?>) errors) {
                            if (error instanceof Throwable
                                ...
                                .getTransactionAttribute() instanceof DelegatingTransactionAttributeWithEither
                                && ((DelegatingTransactionAttributeWithEither) txinfo.getTransactionAttribute())
                                    .rollbackOnErrorValue(error.getClass())) {
                                txinfo.getReactiveTransaction().setRollbackOnly();
                                break;
                            }
                        }
                        ...
                        && ((DelegatingTransactionAttributeWithEither) txinfo.getTransactionAttribute())
                            .rollbackOnErrorValue(errors.getClass())) {
                        txinfo.getReactiveTransaction().setRollbackOnly();
                    }
                }
            });
        ...
        /**
         * Determine whether the given method returns an Either or is wrapped by a reactive type.
         *
         * @param invocation the method invocation
         * @return {@code true} if the method returns an Either or is wrapped by a reactive type
         */
        public static boolean determineEitherType(MethodInvocation invocation) {
            if (Either.class.isAssignableFrom(invocation.getMethod().getReturnType())) {
                return true;
            }

            // Check for a reactive type wrapper (any subtype of org.reactivestreams.Publisher).
            if (invocation.getMethod().getReturnType().getInterfaces().length > 0) {
                for (Class<?> clazz : invocation.getMethod().getReturnType().getInterfaces()) {
                    if (Publisher.class.isAssignableFrom(clazz)) {
                        return Either.class.isAssignableFrom(
                            VavrDelegate.determineRightType(invocation.getMethod().getGenericReturnType()));
                    }
                }
            }
        ...
        /**
         * Determines what the type of the right part of this {@link Either} is.
         *
         * @param type the {@link Either} type
         * @return the type of the right part of the type
         */
        public static Class<?> determineRightType(Type type) {
            if (!(type instanceof ParameterizedType)) {
                throw new IllegalStateException("Type must be a ParameterizedType");
            }

            final Type[] actualTypeArguments = ((ParameterizedType) type).getActualTypeArguments();
            if (actualTypeArguments.length != 1) {
                throw new IllegalStateException("Type must have one type arguments");
            }

            // Get the Class of the right type.
            final Type rightType = actualTypeArguments[0];
            if (rightType instanceof Class) {
                return (Class<?>) rightType;
            } else if (rightType instanceof ParameterizedType) {
                return (Class<?>) ((ParameterizedType) rightType).getRawType();
            } else {
                throw new IllegalStateException("Right type of Type must be a Class or ParameterizedType");
            }
        }
    ...
    private class ReactiveTransactionSupport {
        ...
        public Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
            ...
            if (Mono.class.isAssignableFrom(method.getReturnType()) || (KotlinDetector.isSuspendingFunction(method) &&
                !COROUTINES_FLOW_CLASS_NAME.equals(new MethodParameter(method, -1).getParameterType().getName()))) {
                return TransactionContextManager.currentContext().flatMap(context -> Mono
            ...
            return this.adapter.fromPublisher(TransactionContextManager.currentContext().flatMapMany(context -> Flux
                .usingWhen(
                    createTransactionIfNecessary(rtm, txAttr, joinpointIdentification),
                    tx -> {
                        try {
